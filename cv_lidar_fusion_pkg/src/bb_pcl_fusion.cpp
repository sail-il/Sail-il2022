...
const pcl::PointCloud<pcl::PointXYZ>::Ptr p_pcl){
    std::vector<pcl::PointXYZ, Eigen::aligned_allocator<pcl::PointXYZ>> subarray;
    pcl::PointXYZ av_point;
    int y_delta = (boundingbox->ymax)-(boundingbox->ymin);
    int x_delta = (boundingbox->xmax)-(boundingbox->xmin);
    int ymin_crop = ceil((boundingbox->ymin) + 0.85*y_delta);
    int ymax_crop = ceil((boundingbox->ymax) - 0*y_delta);
    int xmin_crop = floor((boundingbox->xmin) + 0.18*x_delta);
    int xmax_crop = ceil((boundingbox->xmax) - 0.18*x_delta);
    int y_delta_crop = ymax_crop - ymin_crop;
    int x_delta_crop = xmax_crop - xmin_crop;
    int ymin_weighted = ymin_crop + floor(0.25*y_delta_crop)
    int ymax_weighted = ymax_crop - ceil(0.25*y_delta_crop);
    int xmin_weighted = xmin_crop + floor(0.25*x_delta_crop)
    int xmax_weighted = xmin_crop + ceil(0.25*x_delta_crop)
//    for (int i = 0; i < 50;i++){
  //    for( int y = ymin_weighted; y < ymax_weighted; y++){
    //    for( int x = xmin_weighted;x < xmax_weighted; x++){
      //    if(!check_nan_inf(p_pcl->points[y*p_pcl->width + x])) continue;
        //  subarray.push_back(p_pcl->points[y*p_pcl->width + x]);
//          }
  //      } 
  ...
  //    for( int x = xmin_weighted;x < xmax_weighted; x++){
    //    if(!check_nan_inf(p_pcl->points[y*p_pcl->width + x])) continue;
      //  subarray.push_back(p_pcl->points[y*p_pcl->width + x]);
        //}
     // }
    for( int y = ymin_crop; y < ymax_crop; y++){
      for( int x = xmin_crop;x < xmax_crop; x++){
        if(!check_nan_inf(p_pcl->points[y*p_pcl->width + x])) continue;
...
pcl::PointXYZ avg_point(std::vector<pcl::PointXYZ, Eigen::aligned_allocator<pcl::PointXYZ>> subarray)
{
  ...
  int new_av_x = 0;
  for(int i = 0; i < size; i ++){
    if (abs(subarray[i].x-av_point.x)<0.25){
      new_size++;
      new_av_x += subarray[i].x;
    }
  av_point.x = new_av_x/new_size++;
...
/*  avg_x = 0;
 ...
 for(int i = 0; i < size; i ++)
  {
    if ((subarray[i].x > (av_point.x - 1)) && (subarray[i].x < (av_point.x + 1)) 
     && (subarray[i].y > (av_point.y - 1)) && (subarray[i].y > (av_point.y - 1)) 
     && (subarray[i].z > (av_point.z - 1)) && (subarray[i].z > (av_point.z - 1))){
    avg_x += subarray[i].x;
    avg_y += subarray[i].y;
    avg_z += subarray[i].z;
    size += 1;

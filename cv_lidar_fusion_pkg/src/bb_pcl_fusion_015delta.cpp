...
const pcl::PointCloud<pcl::PointXYZ>::Ptr p_pcl){
    ...
    for(int y = floor((boundingbox->ymin)+0.15*y_delta);y < ceil((boundingbox->ymax)-0.15*y_delta); y++){
      for(int x = floor((boundingbox->xmin)+0.15*x_delta);x < ceil((boundingbox->xmax)-0.15*x_delta); x++){
        if(!check_nan_inf(p_pcl->points[y*p_pcl->width + x])) continue;
